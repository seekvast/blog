...
import {
  BoardPermission,
  ROLE_PERMISSIONS,
} from "@/constants/board-permissions";
...
export function usePermission(board?: Board, discussion?: Discussion) {
  ...
  const isAuthor = discussion ? user?.hashid === discussion.user.hashid : false;
  ...
  const hasPermission = (permission: BoardPermission): boolean => {
    ...
    if (isAuthor) {
      if (
        permission === BoardPermission.EDIT_OWN_DISCUSSION ||
    ...
    // 检查用户角色是否有该权限
    return (
      ROLE_PERMISSIONS[userRole as keyof typeof ROLE_PERMISSIONS]?.includes(
        permission
      ) || false
    );
  };
