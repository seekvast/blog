...
import type { Pagination, User, QueryParams } from '@/types/common'
...
export function createApi(options: ApiOptions = {}) {
  ...
  return {
    users: {
      ...
      me: () => 
        http.get<User>(`${prefix}/users/me`, undefined, { next }),
    ...
    boards: {
      ...
      getChildren: (boardId: number) =>
        http.get<BoardChild[]>(`${prefix}/board/children?board_id=${boardId}`, undefined, { next })
    },
    
    discussions: {
      list: (params?: QueryParams) =>
        http.get<Pagination<Discussion>>(`${prefix}/discussions`, params, { next }),
        
      get: (slug: string) =>
        http.get<Discussion>(`${prefix}/discussion?slug=${slug}`, undefined, { next }),
      ...
      delete: (slug: string) =>
        http.delete(`${prefix}/discussions/${slug}`),
      
      posts: (slug: string, params?: QueryParams) =>
        http.get<Pagination<Post>>(`${prefix}/discussions/${slug}/posts`, params, { next }),
    ...
    posts: {
      ...
      create: (data: any) =>
        http.post<Post>(`${prefix}/posts`, data),
        
      update: (slug: string, data: any) =>
        http.patch<Post>(`${prefix}/posts/${slug}`, data),
        
      delete: (slug: string) =>
        http.delete(`${prefix}/posts/${slug}`)
    }
  }
}
